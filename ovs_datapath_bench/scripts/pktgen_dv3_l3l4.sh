...
# Ensure pktgen module is loaded before entering the namespace.
sudo modprobe pktgen

if ! sudo ip netns exec "$NS" test -d /proc/net/pktgen; then
  echo "[ERROR] /proc/net/pktgen is not available inside namespace $NS"
  echo "[HINT] Check: sudo modprobe pktgen && sudo ip netns exec $NS ls -l /proc/net/pktgen"
  exit 1
fi
...
# Device is already selected by add_device $PKTGEN_DEV.
# Do not write "dev $DEV" into $PGPKT; some pktgen versions reject it.
...
# Flags: randomize enough fields to avoid repeatedly hitting the same exact cache path.
echo "flag UDPSRC_RND" > "$PGPKT"
echo "flag UDPDST_RND" > "$PGPKT"
echo "flag IPSRC_RND" > "$PGPKT"
echo "flag IPDST_RND" > "$PGPKT"

# With count 0, "echo start > pgctrl" blocks until pktgen stops.
# Run it in the background, sleep for the requested duration, then stop.
echo "start" > "$PGCTRL" &
START_PID=$!
